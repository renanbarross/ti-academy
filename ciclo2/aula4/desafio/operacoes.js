...
const cestaDoClienteN = document.querySelectorAll("#cestaDoCliente");
const listaProd = document.querySelector("ul#cestaDoCliente");


cestaDoClienteN.forEach((itemEscolhido)=>{

    itemEscolhido.addEventListener('click', (itemS)=>{
                    
        var idx = armazenaItens.indexOf(itemS.target.textContent);
                    
        if (idx > -1) {
                    
            armazenaItens.splice(idx, 1);

            cestaDoCliente.removeChild(listaProd.childNodes[idx]);
                
            totalPedido -= Number(itemS.target.dataset.preco);
